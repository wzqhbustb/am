...
fn test_bootstrap_from_empty_dir() {
    ...
    // All five system tables are present with their PG-conventional OIDs.
    let expected = [
    ...
    for def in [PG_CLASS, PG_ATTRIBUTE, PG_AM] {
...
fn test_half_state_recovery() {
    ...
    // Simulate a crash mid-bootstrap: pages 1..=5 allocated (PageAlloc WAL is
...
fn test_partial_flush_half_state_triggers_rebootstrap() {
    ...
    // Simulate the crashed state: pages 1..=5 allocated; page 1 is a
    // valid-looking slotted page with a pg_class row; pages 2..=5 are zeros.
    {
        let engine = open_engine(tmp.path());
        for expected in 1..=5u64 {
            assert_eq!(
